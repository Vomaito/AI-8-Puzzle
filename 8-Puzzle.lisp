...
(DEFUN ADDOPEN (OPTION)
  ...
  (LOOP
    (SETQ NODO (NTH INDEX OPENSTACK))
    (((NEQ NODO NIL)
      ((< COST (+ (SECOND NODO) (THIRD NODO)) )
      ((= INDEX 0)
        (SETQ OPENSTACK (APPEND (LIST OPTION) OPENSTACK))
      )
        (SETQ
          OPENSTACK (APPEND (FIRSTN INDEX OPENSTACK) (LIST OPTION) (NTHCDR (+ 1 INDEX) OPENSTACK))
        )
      )
    ))
    (INCQ INDEX)
...
(DEFUN ADDCLOSE (OPTION)
  (PUSH OPTION CLOSEDSTACK)
)

(DEFUN ISINCLOSEDSTACK (NODO)
  (SETQ INDEX 0)
  (LOOP
    ((NULL (CAR(NTH INDEX CLOSEDSTACK))) NIL)
    ((EQUAL (CAR NODO) (CAR(NTH INDEX CLOSEDSTACK)))
